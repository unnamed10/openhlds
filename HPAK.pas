...
procedure HPAK_AddLump(ToCache: Boolean; Name: PLChar; Res: PResource; Data: Pointer; F: PFile);
...
if Name = nil then
 ...
 if Res = nil then
  ...
  if not ((Data = nil) xor (F = nil)) then
   ...
   if (Res.DownloadSize < 1024) or (Res.DownloadSize >= HPAK_MAX_DATA_SIZE) then
     ...
     if Data <> nil then
      ...
      begin
       ...
       if FS_Read(F^, P, Res.DownloadSize) < Res.DownloadSize then
        begin
         Print('HPAK_AddLump: File read error.');
         Mem_Free(P);
         Exit;
        end;
     ...
     if not CompareMem(@Res.MD5Hash, @MD5H, SizeOf(MD5H)) then
      ...
      if ToCache then
       ...
       begin
        ...
        if not FS_Open(F1, Name, 'r') then
         HPAK_CreatePak(Name, Res, Data, F)
        else
         begin
          S2 := @Buf2;
          COM_StripExtension(Name, S2);
          COM_DefaultExtension(S2, '.hp2');
          if not FS_Open(F2, S2, 'wo') then
           Print(['HPAK_AddLump: Couldn''t open "', S2, '" for writing.'])
          else
           begin
            if FS_Read(F1, @Header, SizeOf(Header)) < SizeOf(Header) then
             Print(['HPAK_AddLump: File read error.'])
            else
             if PUInt32(@Header.FileTag)^ <> HPAK_TAG then
              Print(['HPAK_AddLump: "', Name, '" is not a HPAK file.'])
             else
              if Header.Version <> HPAK_VERSION then
               Print('HPAK_AddLump: HPAK version mismatch.')
              else
               if (Header.FileOffset < SizeOf(Header)) or (Header.FileOffset >= FS_Size(F1)) then
                Print('HPAK_AddLump: Invalid HPAK file offset.')
               else
                begin
                 FS_Seek(F1, 0, SEEK_SET);
                 COM_CopyFileChunk(F2, F1, FS_Size(F1));
                 ...
                 if (DH.NumEntries < 1) or (DH.NumEntries > HPAK_MAX_ENTRIES) then
                  ...
                  begin
                   DH.Entries := Mem_ZeroAlloc(SizeOf(THPAKDirectory) * DH.NumEntries);
                   if DH.Entries = nil then
                    Print('HPAK_AddLump: Out of memory.')
                   else
                    begin
                     ...
                     if not HPAK_FindResource(@DH, @Res.MD5Hash, nil) then
                      begin
                       ...
                       DH2.Entries := Mem_ZeroAlloc(SizeOf(THPAKDirectory) * (DH.NumEntries + 1));
                       if DH2.Entries = nil then
                        ...
                        begin
                         ...
                         FS_Seek(F2, Header.FileOffset, SEEK_SET);
                         Move(Res^, LE.Resource, SizeOf(LE.Resource));
                         LE.FileOffset := UInt32(FS_Tell(F2));
                         LE.Size := Res.DownloadSize;
                         if Data <> nil then
                          FS_Write(F2, Data, Res.DownloadSize)
                         else
                          COM_CopyFileChunk(F2, F^, Res.DownloadSize);

                         Header.FileOffset := UInt32(FS_Tell(F2));
...
if (Name = nil) or (Name^ = #0) then
 ...
 begin
  ...
  SN := Cmd_Argv(1);
  if not COM_HasExtension(SN) then
   begin
  ...
  if not FS_Open(F1, S, 'r') then
   ...
   begin
    S2 := @Buf2;
    COM_StripExtension(SN, S2);
    COM_DefaultExtension(S2, '.hp2');
    if not FS_Open(F2, S2, 'wo') then
     Print(['HPAK_RemoveLump: Couldn''t create HPAK file "', S2, '".'])
    else
     begin
      ...
      if PUInt32(@Header.FileTag)^ <> HPAK_TAG then
       ...
       if Header.Version <> HPAK_VERSION then
        Print('HPAK_RemoveLump: HPAK version mismatch.')
       else
        if (Header.FileOffset < SizeOf(Header)) or (Header.FileOffset >= FS_Size(F1)) then
         Print('HPAK_RemoveLump: Invalid HPAK file offset.')
        else
         begin
          FS_Seek(F1, Header.FileOffset, SEEK_SET);
          FS_Read(F1, @DH.NumEntries, SizeOf(DH.NumEntries));
...
if HPAK_ParseHeaders(Name, F, Header, DH) then
 begin
  if (Index < 0) or (Index >= DH.NumEntries) then
   ...
   begin
    if Res <> nil then
     Move(DH.Entries[Index].Resource, Res^, SizeOf(Res^));
    Result := True;
   end;
   ...
   begin
    ...
    for I := 0 to DH.NumEntries - 1 do
     begin
      ...
      Print(['#', I, ': ', HPAK_GetRTDesc(E), ', size = ', Size, 'K, name = "', COM_FileBase(@E.Resource.Name, @Buf), '" : ', PLChar(@MD5S)]);
     end;
...
var
 ...
 I: Int;
begin
if (Res.DownloadSize = 0) or (Res.DownloadSize >= HPAK_MAX_DATA_SIZE) then
 Print('HPAK_CreatePak: Bad resource size.')
else
 if not ((Data = nil) xor (F = nil)) then
...
if Cmd_Argc <> 3 then
 ...
 begin
  ...
  if Index < 0 then
   Print('Bad lump index, must be an unsigned integer value.')
  else
...
if Cmd_Argc <> 2 then
 ...
 begin
  ...
  if HPAK_ParseHeaders(Name, F, Header, DH) then
   begin
    Print(['| Number of entries: ', DH.NumEntries, '/', HPAK_MAX_ENTRIES, LineBreak + '| Type Size FileName']);
    for I := 0 to DH.NumEntries - 1 do
     begin
      E := @DH.Entries[I];

      Size := RoundTo(E.Resource.DownloadSize / 1024, -2);
      Print(['#', I, ': ', HPAK_GetRTDesc(E), ', size = ', Size, 'K, name = "', COM_FileBase(@E.Resource.Name, @Buf), '"']);

      if (E.Size = 0) or (E.Size >= HPAK_MAX_DATA_SIZE) then
       Print(['#', I, ': lump size is invalid (', E.Size, ').'])
      else
       begin
    ...
    for I := 0 to DH.NumEntries - 1 do
     if (Index = -1) or (I = Index) then
      begin
       ...
       Size := RoundTo(E.Resource.DownloadSize / 1024, -2);
       Print(['Extracting lump #', I, ': ', HPAK_GetRTDesc(E), ', size = ', Size, 'K, name = "', COM_FileBase(@E.Resource.Name, @Buf), '"']);
       ...
       if (E.Size = 0) or (E.Size >= HPAK_MAX_DATA_SIZE) then
        ...
        begin
         ...
         if FS_Open(F2, @Buf2, 'wo') then
          ...
          Print(['Error creating WAD file "', PLChar(@Buf2), '".']);
...
var
 Buf: array[1..MAX_PATH_W] of LChar;
 DataBuf: array[1..HPAK_MAX_DATA_SIZE] of Byte;
 F: TFile;
 Header: THPAKHeader;
 DH: THPAKDirectoryHeader;
 I: Int;
